...
pub fn build(b: *std.Build) !void {
    ...
    // Module
    const ulid_module = b.addModule("ulid", .{ .root_source_file = b.path("ulid.zig") });

    // Static library
    const ulid_lib = b.addStaticLibrary(.{ .name = "ulid", .root_source_file = b.path("ulid.zig"), .target = target, .optimize = optimize });
    ulid_lib.linkLibC();
    ...
    const ulid_bench = b.addExecutable(.{
        .name = "ulid-bench",
        .root_source_file = b.path("ulid_benchmark.zig"),
        .target = target,
    ...
    const ulid_example = b.addExecutable(.{
        .name = "ulid-example",
        .target = target,
        .root_source_file = b.path("ulid_example.zig"),
    });
    ...
    // CLI
    const ulid_bin = b.addExecutable(.{ .name = "ulid", .target = target });
    ulid_bin.addCSourceFile(.{ .file = b.path("ulid.c"), .flags = &[_][]const u8{} });
    ...
    ulid_test.linkLibC();
    const ulid_ctest = b.addExecutable(.{ .name = "ulidc-test", .target = target });
    ulid_ctest.addCSourceFile(.{ .file = b.path("ulid.c"), .flags = &[_][]const u8{"-DTEST"} });
